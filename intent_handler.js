...
function IntentHandler() {
  ...
  var getStartedText = 'Now let\'s start. How many math questions would you like to have? ',
    ...
    welcomeText = 'Welcome to Jolly Rancher Math Land. ' +
      getSound('game-intro1.mp3') +
      'In Jolly Rancher Math Land, you earn jolly ranchers by playing math games with me. ' +
      'The more math problems you solve, the more jolly ranchers you will earn.  ' +
  ...
  function goodBye(req, res) {
    var goodBye = ' It was a pleasure playing with you. Come to visit Jolly Rancher Math Land soon. Goodbye!';
  ...
  function getResultText(isCorrect, answer, scoreTotal) {
    ...
    if (answer === 'donotknow') {
      return 'It is ok. You now have ' + (scoreTotal).toString() + ' jolly ranchers. ';
    }

    if (isCorrect) {
      result += 'You earned 10 jolly ranchers. ';
    }

    result += 'You now have ' + (scoreTotal).toString() + ' jolly ranchers. ';
  ...
  function getSectionCompleteOutput(scoreTotal) {
    ...
    var totalText = scoreTotal > 0 ? 'Your have earned ' + scoreTotal + ' jolly ranchers. ' :
      'Sorry, you did not earn any jolly ranchers. Don\'t worry, you will do better next time. ';
    var gameOverSound = getSound('game-over1.mp3');

    return gameOverSound + 'Good job. You have completed the Jolly Ranch Math game. '
      + totalText
      + ' It was a pleasure playing with you. Come to visit Jolly Rancher Math Land soon. Goodbye!';
  }
